...
class DeviceOwnerInstaller @Inject constructor(
    ...
    override fun install(download: Download) {
        ...
        if (sessionSet != null) {
            ...
            // Enqueue and trigger installation only if we have sessions
            if (sessionInfoSet.isNotEmpty()) {
                enqueuedSessions.add(sessionInfoSet)
                commitInstall(sessionInfoSet.first())
            } else {
                Log.e(TAG, "No sessions created for ${download.packageName}")
                postError(download.packageName, "No APK files found", null)
            }
        }
    }
    ...
    private fun buildSessionParams(packageName: String): SessionParams =
        SessionParams(SessionParams.MODE_FULL_INSTALL).apply {
            ...
            if (isOAndAbove) {
                // Use INSTALL_REASON_POLICY for device owner installations
    ...
    private fun commitSession(sessionInfo: SessionInfo) {
        try {
            ...
            val callbackIntent = getCallBackIntent(sessionInfo)
            if (callbackIntent != null) {
                session.commit(callbackIntent.intentSender)
            } else {
                Log.e(TAG, "Failed to create callback intent for ${sessionInfo.packageName}")
                postError(sessionInfo.packageName, "Failed to create callback intent", null)
            }
            session.close()
        } catch (e: Exception) {
            Log.e(TAG, "Error committing session: ${e.message}")
